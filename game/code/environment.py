...
def run_program():
    ...
    while running:
        ...
        if keys[pygame.K_w] and player_rect.y>=80:
            player_rect.y -= data_module.speed
        if keys[pygame.K_s] and player_rect.y<=1020:
            player_rect.y += data_module.speed
        if keys[pygame.K_a] and player_rect.x>=-10:
            player_rect.x -= data_module.speed
        if keys[pygame.K_d] and player_rect.x<=1550:
            player_rect.x += data_module.speed
        ...
        if (player_rect.x,player_rect.y) in testlist2:
            if number_guan==1:
                ...
                # print(resurrect)
                coins_number=coin_number.increase_coin(300)
                number_run_game+=1
                screen=pygame.display.set_mode((data_module.width,data_module.height))
